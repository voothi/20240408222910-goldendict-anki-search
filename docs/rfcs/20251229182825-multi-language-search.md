...
2.  **Language Variant Expansion**:
    ...
    - Example: `en` expands to `['en-gb', 'en-us', 'en-au', 'en-ca']` (base code removed).
    - This ensures that searching for "en" catches all specific English regional tags.
3.  **Search Logic**:
...
## Refinements
1.  **Base Language Code Removal**: Removed base language codes (e.g., `'en'`, `'de'`) from `LANGUAGE_VARIANTS` lists, keeping only specific regional variants. This prevents overly broad wildcard matches that caused Anki query parsing issues when combined with long OR chains.
2.  **Python 3.9 Compatibility**: Replaced Python 3.10+ union type syntax (`list[str] | None`) with `typing.Optional[List[str]]` to support Python 3.9.
